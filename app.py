# app.py
# Tamamlananlar:
# - Giriş Yap / Kayıt Ol ekranı eklendi
# - Sidebar'a kullanıcı bilgisi ve Çıkış Yap butonu eklendi
# - Dashboard (toplam/bekleyen/tamamlanan görev metrikleri, ilerleme çubuğu) tamamlandı
# - Görev Yönetimi sekmesi: ekleme, filtreleme, tamamlama, silme tamamlandı
# - Kişisel Notlar sekmesi: ekleme, listeleme, silme tamamlandı
# - Performans Analizi sekmesi: tamamlama oranı ve kategori/öncelik grafikleri tamamlandı
# - Pomodoro Sayacı (başlat/duraklat/sıfırla) tamamlandı
# - Bugün/yarın teslim uyarı sistemi ve masaüstü bildirimi entegrasyonu tamamlandı
# - Tüm görev/not işlemleri kullanıcıya özel (username) hale getirildi
...
# Browser notification helper (works both locally and on Streamlit Cloud,
# because it asks the user's browser for permission instead of the OS)
...
try:
    ...
    if has_urgent:
        ...
        with col_notif1:
            if st.button("🔔 Bildirim Gönder (Demo)", use_container_width=True):
                notif_msg = ""
                if today_urgent_tasks:
                    today_titles = ", ".join([t.title for t in today_urgent_tasks[:2]])
                    notif_msg += f"Bugün ({len(today_urgent_tasks)}): {today_titles}. "
                ...
                st.success("Bildirim gönderildi! Tarayıcınız izin istediyse 'İzin Ver'e tıklayın.")
except Exception as e:
